...
const EditTransaction: React.FC = () => {
  ...
  const [movementType, setMovementType] = useState<number>(transaction ? (transaction.type === 'income' ? 1 : transaction.type === 'expense' ? 2 : transaction.type === 'payment' ? 3 :  0) : 0);
  const [selectedCard, setSelectedCard] = useState<number>(transaction ? (transaction.account.id) : 0);
  const [selectedCardA, setSelectedCardA] = useState<number>(transaction ? transaction.account.id : 0);
  const [selectedCardB, setSelectedCardB] = useState<number>();
  const [selectedCardType, setSelectedCardType] = useState<String>(transaction ? transaction.account.type : "");
  const [amount, setAmount] = useState<number>(transaction ? transaction.amount : 0);
  const [transacctionDate, setTransactionDate] = useState<string>(transaction ? transaction.transaction_date : "");
  const [accountingDate, setAccountingDate] = useState<string>(transaction ? transaction.accounting_date : "");
  const [selectedCategory, setSelectedCategory] = useState<number>(transaction ? transaction.category.id : 0);
  const [concept, setConcept] = useState<string>(transaction ? transaction.concept : "");
  const [place, setPlace] = useState<string>(transaction ? transaction.place : "");
    ...
    if (movementType !== null && selectedCategory !== null && selectedCard !== null && concept !== "" && amount !== null && transacctionDate !== "") {
        ...
        try {
          ...
          if (response.ok) {
            ...
            console.log(transaction.accounting_date.slice(0,4))
            console.log(transaction.accounting_date.slice(6,7))
